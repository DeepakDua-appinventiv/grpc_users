...
function makeTimestampMethods(options, longs, bytes) {
    ...
    if (options.forceLong === options_1.LongOption.LONG) {
        ...
        toNumberCode = (0, ts_poet_1.code) `${bytes.globalThis}.Number(t.seconds.toString())`;
        seconds = (0, ts_poet_1.code) `BigInt(Math.trunc(date.getTime() / 1_000))`;
    }
...
function generateOneofProperty(ctx, messageDesc, oneofIndex, sourceInfo) {
    ...
    return (0, ts_poet_1.code) `${mbReadonly}${name}?: ${unionType} | undefined,`;
    /*
...
function generateDecode(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    `;
        ...
        const initializerNecessary = !options.initializeFieldsAsUndefined && (0, types_1.isOptionalProperty)(field, messageDesc.options, options);
...
function getEncodeWriteSnippet(ctx, field) {
    ...
    if ((0, types_1.isEnum)(field) && options.stringEnums) {
        ...
        switch (fieldType) {
            ...
            case "sint64":
            case "sfixed64":
                return (place) => (0, ts_poet_1.code) `if (BigInt.asIntN(64, ${place}) !== ${place}) {
        ...
        writer.uint32(${tag}).${(0, types_1.toReaderCall)(field)}(${place}.toString())`;
            case "uint64":
            case "fixed64":
                return (place) => (0, ts_poet_1.code) `if (BigInt.asUintN(64, ${place}) !== ${place}) {
          throw new Error('value provided for field ${place} of type ${fieldType} too large');
        }
        writer.uint32(${tag}).${(0, types_1.toReaderCall)(field)}(${place}.toString())`;
            default:
        ...
        `;
          ...
          `);
            ...
            if (!processedOneofs.has(field.oneofIndex)) {
                ...
                chunks.push((0, ts_poet_1.code) `switch (message.${oneofName}?.$case) {`);
...
function generateFromJson(ctx, fullName, fullTypeName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        const noDefaultValue = !options.initializeFieldsAsUndefined && (0, types_1.isOptionalProperty)(field, messageDesc.options, options);
        // and then use the snippet to handle repeated fields if necessary
        if ((_a = canonicalFromJson[fullTypeName]) === null || _a === void 0 ? void 0 : _a[fieldName]) {
          ...
          `);
                ...
                if (readValueSnippet.toString() === (0, ts_poet_1.code) `e`.toString()) {
                    chunks.push((0, ts_poet_1.code) `${fieldName}: ${ctx.utils.globalThis}.Array.isArray(${jsonPropertyOptional}) ? [...${jsonProperty}] : [],`);
                }
                else {
